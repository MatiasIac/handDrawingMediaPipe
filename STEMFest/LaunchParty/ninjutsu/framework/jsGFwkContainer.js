	...
	function container(settings) {
		...
		this.update = function (delta) { 
			for (var o in _allObjects) {
				...
				else if (_allObjects[o].update !== undefined) { _allObjects[o].update(delta); }
			}
		};
		this.draw = function (context) { 
			for (var o in _allObjects) {
				if (_allObjects[o].onDraw !== undefined) { _allObjects[o].onDraw(context); }
				else if (_allObjects[o].draw !== undefined) { _allObjects[o].draw(context); }
			}
		};
		...
		this.clearAll = function () {
			for (var o in _allObjects) { 
				if (_allObjects[o].destroy !== undefined) { _allObjects[o].destroy(); } 
			}
			_allObjects = {};
			_objectCounter = 0;
		};
		...
		this.cloneObject = function (initParameters) {
			...
			else if (cloned.init != undefined) { cloned.init(initParameters); }
